...
def ships_placement(board, board_size, coordinates, alphabet, active_player):
    ...
    while len(ships) > 0:
        ...
        user_input = input(
            f"{active_player} please place your ships on the map. Available ships for placement: {ships}. Current ship being placed: {list_ships[0]}. Please select a valid coordinate: ")
        ...
        if user_input.upper() in coordinates.keys():
            ...
            if discrete_board[row][col] == "-":
                ...
                if is_in_the_board(row, col, ship_len, user_input_orientation, coordinates):
                    sunken_ship = mark_ship(board, discrete_board, coordinates, user_input_orientation, row, col,
                                            ship_len)
                    mark_ship_discrete_board(discrete_board, coordinates, user_input_orientation, discrete_row,
                                             discrete_col, ship_len)
...
def ships_placement_AI(board, board_size, coordinates, alphabet, active_player):
    ...
    while len(ships) > 0:
        ...
        if discrete_board[row][col] == "-":
            if is_in_the_board(row, col, ship_len, user_input_orientation, coordinates):
                sunken_ship = mark_ship_ai(board, discrete_board, coordinates, user_input_orientation, row, col,
                                           ship_len)
                mark_ship_discrete_board(discrete_board, coordinates, user_input_orientation, discrete_row,
                                         discrete_col, ship_len)
                ships[list_ships[0]] -= 1
...
def battleships_Human_Human():
    ...
    board_player_1, sunken_ships_coordinates_1 = ships_placement(board, board_size, coordinates, alphabet,
                                                                 active_player)
    board = init_board(board_size)
    active_player = change_active_item(player_1, player_2, active_player)
    board_player_2, sunken_ships_coordinates_2 = ships_placement(board, board_size, coordinates, alphabet,
                                                                 active_player)
    ...
    while is_won(active_shooting_board, active_player_board) == False:
        gameplay(active_player, active_shooting_board, coordinates, active_player_board, board_size, alphabet,
                 active_sunken_ships_coordinates, turn)
        ...
        active_shooting_board = change_active_item(shooting_board_player_1, shooting_board_player_2,
                                                   active_shooting_board)
        active_sunken_ships_coordinates = change_active_item(sunken_ships_coordinates_1, sunken_ships_coordinates_2,
                                                             active_sunken_ships_coordinates)
...
def battleships_Human_AI():
    ...
    board = init_board(board_size)
    active_player = change_active_item(player_1, player_2, active_player)
    board_player_2, sunken_ships_coordinates_2 = ships_placement_AI(board, board_size, coordinates, alphabet,
                                                                    active_player)
    shooting_board_player_1 = init_board(board_size)
    shooting_board_player_2 = init_board(board_size)
    active_player = change_active_item(player_1, player_2, active_player)
...
def battleships_AI_Human():
    ...
    clear()
    active_player = player_1
    board_player_1, sunken_ships_coordinates_1 = ships_placement_AI(board, board_size, coordinates, alphabet,
                                                                    active_player)
    board = init_board(board_size)
    active_player = change_active_item(player_1, player_2, active_player)
    board_player_2, sunken_ships_coordinates_2 = ships_placement(board, board_size, coordinates, alphabet,
